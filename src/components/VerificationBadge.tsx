...
export interface VerificationStats {
  ...
  failedResources?: string[];  // List of paths that failed verification
}
        ...
        <div className="absolute top-full right-0 mt-2 w-72 bg-container-L2 border border-stroke-high rounded-lg shadow-lg p-4 z-50">
          <div className="text-sm text-text-high mb-3">
            ...
            {stats.failedResources && stats.failedResources.length > 0 && (
              <div className="mt-3 pt-3 border-t border-stroke-low">
                <div className="text-xs text-text-low mb-2">Failed Resources:</div>
                <div className="space-y-1 max-h-32 overflow-y-auto scrollbar-styled">
                  {stats.failedResources.map((path, index) => (
                    <div key={index} className="flex items-center gap-1.5 text-xs">
                      <svg className="w-3 h-3 text-semantic-error flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                      </svg>
                      <span className="font-mono text-semantic-error truncate" title={path}>
                        {path}
                      </span>
                    </div>
                  ))}
